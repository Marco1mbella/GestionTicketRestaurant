...
$req=$BD->prepare("SELECT c.id as idCli ,u.email, c.nom_client, t.id as idTicket,t.dateOuverture,t.dateConsultation, t.dateFermeture, t.etatTicket,t.repTicket, cmd.id as idCmd FROM utilisateur u Inner JOIN client c on u.id=c.idUser Inner JOIN tickets t on c.id=t.idClient Inner JOIN commande cmd on c.id=cmd.id_client ORDER BY t.etatTicket");
   $req->execute(); 
   $LisTic=$req->fetchALL();
}catch(Exception $e){
  ...
  ?>
   <tr class="item" id='<?=htmlspecialchars($row["idTicket"])?>'style="text-align=center;" >
    ...
    <td class="form-group"> 
      ...
      <button  class="deleteBT btn btn-danger form-control"  data-id='<?=htmlspecialchars($row["idTicket"])?>' title="Annuler Ticket :'<?=htmlspecialchars($row["idTicket"])?>'" data-toggle="modal" data-target="#modaldeleteT"  <?=htmlspecialchars($statBuCan)?>>
