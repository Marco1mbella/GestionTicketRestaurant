...
$req=$BD->prepare("SELECT c.id as idCli ,u.email, c.nom_client, t.id as idTicket,t.dateOuverture,t.dateConsultation, t.dateFermeture, t.etatTicket,t.repTicket, cmd.id as idCmd FROM utilisateur u Inner JOIN client c on u.id=c.idUser Inner JOIN tickets t on c.id=t.idClient Inner JOIN commande cmd on c.id=cmd.id_client");
   $req->execute(); 
   $LisTic=$req->fetchALL();
}catch(Exception $e){
